...
# --- Targets ---
.PHONY: help detect-arch pulumi-login pulumi-up up talos-gen-config talos-cluster kind-cluster clean clean-all act devcontainer test-kind test-talos stop
...
help:
	...
	@echo "  devcontainer        - Maintain .github/devcontainer submodule."
...
kind-cluster:
	...
	@sudo docker volume create kind-worker-n01
	@sudo docker volume create kind-control-plane-n01
	@sudo kind create cluster --wait 1m --retain --config=hack/kind.yaml
	@sudo kind get clusters
	@sudo kind get kubeconfig --name kind | tee ${KUBE_CONFIG_FILE} 1>/dev/null
	@sudo kind get kubeconfig --name kind | tee ${HOME}/.kube/config 1>/dev/null
...
clean-all: clean
	...
	@sudo docker volume rm kind-worker-n01 kind-control-plane-n01 \
...
devcontainer:
	@echo "Updating devcontainer Devcontainer..."
	@docker pull ghcr.io/pulumi/devcontainer:latest 1>/dev/null &
	@git submodule update --init .github/devcontainer
	@git submodule update --remote --merge .github/devcontainer
	@mkdir -p .devcontainer
	@rsync -av .github/devcontainer/.github/devcontainer/* .devcontainer/
	@echo "Devcontainer is up to date."
